...
static void VisionThread()
    {
		...
		frc::PWM* gimbalPan = new frc::PWM(0);
		frc::PWM* gimbalTilt = new frc::PWM(1);

		gimbalPan->SetRaw(127);
		gimbalTilt->SetRaw(127);
...
//							}
				...
				if (redLights.size() > 0) {
					int gimbalX = gimbalPan->GetRaw()+(int)((float)(centers[redLights[0]].x-source.cols*0.5)/(float)(source.cols)*40);
					if (gimbalX > 255) {
						gimbalX = 255;
					}
					else if (gimbalX < 0) {
					...
					gimbalPan->SetRaw(gimbalX);

					int gimbalY = gimbalTilt->GetRaw()+(int)((float)(centers[redLights[0]].y-source.rows*0.5)/(float)(source.rows)*40);
					if (gimbalY > 255) {
						gimbalY = 255;
					}
					else if (gimbalY < 0) {
...
void Robot::RobotInit() {
	...
	chooser.AddDefault("Autonomous Command", new AutonomousCommand(1, 1, 1, 1, 1));

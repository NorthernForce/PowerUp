...
RobotNavigation::RobotTrajectory RobotNavigation::CreateProfile(const double distance, const double startVelocity, const double finalVelocity, const double angleDegrees) {
    ...
    const auto outsideDistance = distance + std::abs(angleDegrees) / 360 * 3.1416 * wheelTrack;
    const auto insideDistance = distance - std::abs(angleDegrees) / 360 * 3.1416 * wheelTrack;
...
double RobotNavigation::CalculateDuration(const double distance, const double startVelocity, const double finalVelocity)
{
	...
	TrajectoryPoint TrajectoryDuration; // Created a specific object for use with the following return statement
	...
	return RoundProfileDuration(duration, TrajectoryDuration.timeDur = 40);
